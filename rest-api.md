# 币安官方 Rest API 文档 (最后一次更新：2018-01-14)
...
# 限制
*  `/api/v1/exchangeInfo` `rateLimits` 这两个数组包含了交易所请求（`REQUESTS`）和下单 (`ORDER` )的频次限制.
* 当请求次数或下单次数的限制被打破是，会返回429的错误码.
* 每一条线路都存在一个 `weight` 用来决定每一个端点发送的请求次数。承载请求次数多的端点会有更重的 `weight` .
* 当你看到 429，你有责任和义务停止进行API的请求，不滥用API服务. 
* **重复的打破请求限制次数可能会让你的 IP 地址被系统自动屏蔽(418状态).**
* 屏蔽 IP 会被记录然后根据打破请求限制的严重程度，限制**2分钟到3天**
